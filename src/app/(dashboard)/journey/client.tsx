  ...
  async function generateIdeas(touchpoint: Touchpoint) {
    ...
    try {
      ...
      if (!isPaid) {
        setUsed((prev) => {
          const next = prev + 1
          if (next >= FREE_BLUEPRINT_GENERATIONS) setLimitReached(true)
          return next
        })
      }
    } catch {
    ...
    <div className="flex flex-col gap-10 max-w-2xl">
        ...
        <div className="bg-primary/10 border border-primary/20 rounded-xl px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-foreground">
              {limitReached
                ? 'You\'ve used your free idea generations'
                : `Your Blueprint is yours to build — free`}
            </p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {limitReached
                ? 'Upgrade to Legendary for unlimited generation, advanced customization, saved variations, and enhanced outputs.'
                : `Map every touchpoint and note what you do today. You have ${freeRemaining} of ${FREE_BLUEPRINT_GENERATIONS} free idea generations left — Legendary makes them unlimited.`}
            </p>
          </div>
          <Link
            href="/pricing"
      ...
      {grouped.map(({ phase, items }) => (
        <div key={phase} className="flex flex-col gap-3">
          ...
          {items.map((t) => {
            const s = states[t.id]
            const genLocked = !isPaid && limitReached && !s.ideas
            return (
                ...
                {s.expanded && (
                  <div className="px-4 pb-5 flex flex-col gap-4 border-t border-border bg-card/50">
                    ...
                    {genLocked ? (
                      <Link
                        href="/pricing"
                        className="text-sm font-medium text-primary underline underline-offset-2 w-fit"
                      >
                        Free idea generations used — unlock unlimited with Legendary →
                      </Link>
                    ) : (
                      <Button
